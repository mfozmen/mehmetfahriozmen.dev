...
- **Diagram production is owned by the `lab-day` agent** (`.claude/agents/lab-day.md`, Part 2) — it defines the visual style (deep navy, cool blue, off-white, grain), authors SVG, renders WebP to `public/lab/[slug]/` via `.claude/skills/lab-diagrams/render.mjs`, and embeds the image with proper alt text. Don't hand-craft diagram styling in the main conversation.
