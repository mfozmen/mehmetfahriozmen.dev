...
The obvious objection is that maps rot. Somebody writes the ownership file, the rules, the decision log, and six months later it describes a system that no longer exists. That is true of every map kept by hand. So on that project I stopped keeping it by hand.

Four agents run on every pull request before it is pushed. None of them writes feature code. Each one keeps one layer of the map true.

The first traces the blast radius. It follows every caller, async consumer, cache and schema the diff can reach, then runs the full test suite to prove they still work. The second attacks the design against the rules that must hold, the ones the domain cannot break and the load the system has to survive. For each rule it asks where it is enforced, in the database or in application code that can race. The third writes the decisions down. Every design choice that lands gets an entry in the decision log, with what it gave up and why that was acceptable, in the same pull request that made it.

The fourth keeps the test cases for the user journeys, and it is not allowed to read the implementation. It works from the user stories, the same kind *The First Button* was about. A test case written from the code cannot fail, because it asserts what was built. A test case written from the story can, and that is its whole value.
...
One detail surprised me. An agent has no memory between runs, so each of these keeps a small notebook per branch: what it reviewed last time and what it left open. Without that, a reviewer on its fifth pass either reads the whole branch again or forgets what it asked for.
...
A better model does not take that step for you. It goes deeper into what it already has. The agents that impressed me were impressive because somebody had let them read sideways.

This is also why the same tools give different teams different results. DORA measured it in 2025: with AI, throughput goes up, stability goes down, and the tooling amplifies whatever the team already was.<sup id="ref-9" className="scroll-mt-24">[9](#src-9)</sup> A team that knows what its software means and who owns what gets a faster version of itself. A team that does not gets the same agent, and more of its own problems.
...
9. [2025 DORA State of AI-assisted Software Development](https://dora.dev/dora-report-2025/) — Google Cloud, 2025. Survey; correlational.
