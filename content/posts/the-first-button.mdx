...
An acceptance criterion is that instruction one layer up, aimed at behavior instead of a function. It's a sentence about the finished thing that somebody who wasn't in the room can check. There is no code yet and there may be no design yet, and the sentence is still specific enough to fail against. The moment for it is exactly here: after everyone agrees what the story is, before anyone starts building it.
...
Writing it is also how the questions arrive. Nobody thinks about a 60,000-row export while describing the feature. Somebody thinks about it while writing the fourth line, three days before the sprint, which is the cheapest moment that question will ever be asked.
...
Same case, more scaffolding. One of these is executable if something is there to execute it, and both are worth exactly as much as the thinking that went into the line. The grammar is optional. The sentence is not.
...
Nothing about any of this is expensive. It took less time to write than the meeting where we argued about the estimate. And it is a test in the only sense that matters — it can be wrong, out loud, in front of everyone, before a line of code exists.
